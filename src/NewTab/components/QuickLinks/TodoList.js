...
const CheckboxList = () => {
  ...
  const checkAndUpdateDate = () => {
    const storedData = JSON.parse(localStorage.getItem('checkboxItems')) || { items: [], lastReset: null };
    const today = new Date().toISOString().split('T')[0];

    if (storedData.lastReset !== today) {
      // Reset checkboxes and update lastReset date
      const resetItems = storedData.items.map(item => ({ ...item, checked: false }));
  ...
  const addItem = (currentIndex) => {
    ...
    const newItems = [...items.slice(0, currentIndex + 1), newItem, ...items.slice(currentIndex + 1)];
    setItems(newItems);
  ...
  return (
      <div className="inline-block min-w-[400px] max-w-[900px] bg-gray-200 p-2 rounded-lg shadow-md dark:bg-gray-700/80 opacity-50 hover:opacity-80 transition-all">
          <ul>
            {items.map((item, index) => (
                <li key={index} className="flex items-center mb-1">
                    ...
                    <button
                        ...
                        title="Add Item"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-3 w-3 text-red-700 dark:text-red-100">
              ...
              <div className="relative flex items-center">
                {
                  items.length <= 9 && <button
                    ...
                    title="Add Item"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-4 w-4 text-black dark:text-white">
                ...
                {/* Popup Content */}
                  {
                      showTodoListInfoContent && <div className="absolute right-6 w-64 mb-2 group-hover:block">
                        <div className="bg-gray-900 text-white text-sm rounded py-2 px-4 shadow-lg">
                            This daily todo list is meant to serve as repeated daily reminders, so we'll uncheck everything at midnight every day.
                        </div>
                    </div>
                }
            </div>
